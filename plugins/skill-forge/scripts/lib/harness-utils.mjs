...
// Shared utilities for create-harness.mjs and validate-harness.mjs:
// project detection, template copying, harness scoring, HTML rendering.
...
export function initScriptFromCommands(commands) {
  const body = commands
    .map((command) => `echo "=== ${escapeForEcho(command)} ==="\n${command}`)
    .join('\n\n');
  return `#!/bin/bash
set -e
...
export function scoreHarness(files) {
  ...
  const checks = {
    instructions: [
      ...
      textHas(
        agents,
        ['Verification Commands', './init.sh', 'test', 'verify'],
