...
- **Ported scripts carry origin-repo path/layout assumptions, undetected by structural validators (occurrence 2 of ported-content drift).** 0.8.0 logged "no external-sources index"; this run found the concrete failure mode (broken runtime path shipped + survived a release). At occurrence 3, the case for a port-integration smoke test in CI becomes strong.
  - **Follow-up (0.9.1):** the concrete CI smoke test (`scripts/smoke-harness.mjs`, wired into `.github/workflows/ci.yml`) was added **proactively** — it executes `create-harness.mjs` against four stack fixtures and asserts the generated `init.sh`, exactly what would have caught the `TEMPLATE_DIR` bug. This is an engineering safeguard, NOT a knowledge-base mutation, so it does not satisfy the 3-occurrence gate: the _reference-doc generalization_ (a Golden Rule in `agentic-subsystems.md` mandating end-to-end execution of ported scripts, plus a `shared/external-sources.md` index) remains **held at occurrence 2**. A future run hitting this pattern should still count it as occurrence 3, not treat it as resolved.
